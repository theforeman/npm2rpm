...
function generateSpecFile(npm_module, dependencies, release, template) {
  ...
  spec_file = replaceAttribute(spec_file, '\\$LICENSE_TYPE', npm_module.license);
  ...
  if (npm_module.license_file.length !== 0) {
    spec_file = replaceAttribute(spec_file, '\\$LICENSEFILE', '%license ' + npm_module.license_file);
  } else {
    spec_file = replaceAttribute(spec_file, '\\$LICENSEFILE', '');
  }
  ...
  if (npm_module.bundle) {
    ...
    spec_file = replaceAttribute(spec_file, '\\%nodejs_symlink_deps', '');
  } else {
    spec_file = replaceAttribute(spec_file, '\\$SETUP', '%setup -q -n package');
    spec_file = replaceAttribute(spec_file, '\\$SOURCES', 'Source0: https://registry.npmjs.org/%{npm_name}/-/%{npm_name}-%{version}.tgz');
