...
function copyFiles(npm_module){
  var result = '';
  if (npm_module.bundle)
    result += 'cd node_modules/%{npm_name}\n';
...
function generateSpecFile(npm_module, dependencies, release, template) {
  ...
  if (npm_module.bundle) {
    ...
    spec_file = replaceAttribute(spec_file, '\\$SETUP', sourcesToCache());
  } else {
...
function npmInstallCache() {
  return 'npm install --cache-min 9999999 --cache . %{npm_name}';
}

function sourcesToCache() {
...
\n\
%setup -T -q -a $(($count-1)) -D -n npm_cache';
}
