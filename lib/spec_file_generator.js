...
function listDocFiles(doc_files) {
  var result = doc_files.map((doc_file) => {
		return '%doc %{nodejs_sitelib}/%{npm_name}/' + doc_file;
	});
	return result.join('\n');
}
...
function generateSpecFile(npm_module, dependencies, release, template) {
	...
	spec_file = replaceAttribute(spec_file, '\\$LICENSEFILE', '%doc %{nodejs_sitelib}/%{npm_name}/' + npm_module.license_file);
  ...
  if (npm_module.bundle) {
    ...
    spec_file = replaceAttribute(spec_file, '\\$BUILD', npmInstallCache());
    spec_file = replaceAttribute(spec_file, '\\$SETUP', sourcesToCache(dependencies));
    spec_file = replaceAttribute(spec_file, '\\$NODEJS_SYMLINK_DEPS_CHECK', '#%{nodejs_symlink_deps} --check');
    spec_file = replaceAttribute(spec_file, '\\$NODEJS_SYMLINK_DEPS', '#%nodejs_symlink_deps');
  } else {
...
function npmInstallCache() {
  return 'npm install --cache-min Infinity --cache . %{npm_name}@%{version} --prefix %{_builddir}/npm_cache';
}
